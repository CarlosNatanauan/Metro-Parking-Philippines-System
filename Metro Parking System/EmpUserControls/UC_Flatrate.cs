...
namespace Metro_Parking_System.UserControls
{
    public partial class UC_Flatrate : UserControl
    {
        ...
        public UC_Flatrate()
        {
            ...
            //printDocument1.DefaultPageSettings.Margins = new Margins(ConvertMillimetersToPixels(4), ConvertMillimetersToPixels(4), 0, 0);
        }

        private void btInFlatrate_Click(object sender, EventArgs e)
        {
            DateTime timeIn = DateTime.Now;
            string plateNumber = tbplateNoFlatrate.Text;
            ...
            if (!databaseCodes.InsertPlateNumberFlatrate(plateNumber, name, vehicleType, timeIn))
            {
                MessageBox.Show("All flatrate parking places are full.", "Parking Full", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        private void UC_Flatrate_Load_1(object sender, EventArgs e)
        {
        ...
        private void ClearBoxes() {
            tbplateNoFlatrate.Text = string.Empty;
        ...
        private decimal vatReceipt = 0;
            ...
            private void btOutFlatrate_Click(object sender, EventArgs e)
            {
                if (dgvFlatrate.SelectedRows.Count > 0)
                {
                    ...
                    int selectedRowIndex = dgvFlatrate.SelectedRows[0].Index;
                    int idColumnIndex = 6;
                    int id = Convert.ToInt32(dgvFlatrate.Rows[selectedRowIndex].Cells[idColumnIndex].Value);
                    idReceipt = id;
                    DataRow selectedRow = databaseCodes.GetFlatrateRecordById(id);

                    if (selectedRow != null)
                    {
                        string plateNumber = selectedRow["plateNumber"] != DBNull.Value ? selectedRow["plateNumber"].ToString() : string.Empty;
                        string selectedPlace = databaseCodes.GetPlaceForPlateNumberFlatrate(plateNumber);
                        ...
                        decimal rate = selectedRow["rate"] != DBNull.Value ? Convert.ToInt32(selectedRow["rate"]) : databaseCodes.RateFlatEntry;


                        decimal tempRateFlatrate = databaseCodes.CalculateUpdatedRateFlatrate(plateNumber, false, false); // Default values assuming no ticket is lost and no discount is applied
                        using (EnterCash enterCashForm = new EnterCash())
                        {
                            enterCashForm.SetTemporaryRate(tempRateFlatrate); // Assuming you have a method to display the temporary rate on the form

                            enterCashForm.RateChanged += (isDiscountApplied, isTicketLost) =>
                            {
                                // Recalculate the temporary rate based on new checkbox states
                                decimal newTempRate = databaseCodes.CalculateUpdatedRateFlatrate(plateNumber, isTicketLost, isDiscountApplied);
                            ...
                            if (result == DialogResult.OK)
                            {
                                ...
                                rate = databaseCodes.CalculateUpdatedRateFlatrate(plateNumber, isTicketLost, isDiscountApplied);

                                databaseCodes.InsertTimeOutForFlatrate(plateNumber, timeOut, isTicketLost, isDiscountApplied);
                                ...
                                if (!decimal.TryParse(receiptValues["Regular Rate:"].Replace("P", "").Trim(), out decimal regularRateReceiptValue))
                                {
                                ...
                                if (enterCashForm.IsPaidButtonClicked)
                                {
                                    ...
                                    int idDeleted = databaseCodes.DeleteFromFlatrateRaw(plateNumber);
                                    databaseCodes.InsertFlatrateLog(id, plateNumber, name, vehicleType, timeIn, timeOut, rate, selectedPlace, discountInfo);
                                    dgvFlatrate.DataSource = databaseCodes.GetAllPlateNumbersFlatrate();
                                    ShowPrintPreviewReceipt(id);//for printing
                                    databaseCodes.IncrementPlaceValueFlatrate(selectedPlace);
                                    Refresher.Enabled = true;
                                    databaseCodes.UpdateRefreshTimestamp(DateTime.Now);
        ...
        private void btSearchFlatrate_Click(object sender, EventArgs e)
        {
            string searchPlateValue = tbPlateFlatrate.Text.Trim();
            string searchIdValue = tbIdFlatrate.Text.Trim();
            ...
            tbPlateFlatrate.Text = string.Empty;
            tbIdFlatrate.Text = string.Empty;
            ClearBoxes();
        }
        ...
        private void Refresher_Tick(object sender, EventArgs e)
        {
            ...
            {
                dgvFlatrate.Invoke(new Action(() => dgvFlatrate.DataSource = databaseCodes.GetAllPlateNumbersFlatrate()));
                UpdateOccupiedAndAvailableLabels();
                lastUpdated = timestampNullable.Value;
            }
            // Optionally handle the case where timestampNullable does not have a value.
        }
        ...
        private void printDocument1_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            // Print TIME IN
            Font boldFont = new Font("Courier New", 6, FontStyle.Bold); // Adjust the size and style as per your needs
            ...
            string placeFromDatabase = databaseCodes.GetPlaceForPlateNumberFlatrate(tbplateNoFlatrate.Text.Trim()).ToUpper();

            string[] values =
            {
            cbVehicleTypeFlat.SelectedItem.ToString(),
            tbplateNoFlatrate.Text.ToUpper(),
            tbNameFlat.Text,
            placeFromDatabase
            };
            ...
            {
                Font detailFont = new Font("Courier New", 6, FontStyle.Regular); // Adjust size as necessary
        ...
        private void DisableControlsBasedOnJobDesc()
        {
            ...
            switch (employee.jobDesc)
            {
                case "Exit Lead":
                    ...
                    tbplateNoFlatrate.Enabled = false;
                    cbVehicleTypeFlat.Enabled = false;
                    tbNameFlat.Enabled = false;
                    btbClearFlatrate.Enabled = false;
                    btInFlatrate.Enabled = false;
                    break;

                case "Entry Lead":
                    tbPlateFlatrate.Enabled = false;
                    tbIdFlatrate.Enabled = false;
                    btSearchFlatrate.Enabled = false;
                    btOutFlatrate.Enabled = false;
                    break;
            }
        }


        private void tbSearchFlatrate_TextChanged(object sender, EventArgs e)
        {

        }

        private void tbplateNoFlatrate_TextChanged(object sender, EventArgs e)
        {
        ...
        {
            var (rate_flat_entry, others_pen_loss_tix, others_pen_over_stay, others_dis_pwdsen) = databaseCodes.GetRatesAndPenaltiesFlatrate();

            decimal regularRate = rate_flat_entry;
            ...
            decimal originalRate = regularRate + overNightFee + lostTicketFee;
            decimal discountAmount = originalRate - rate;
        ...
        private void printDocument2_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            {
                DataRow row = databaseCodes.GetFlatrateRecordByIdLog(id);
        ...
        private void dgvFlatrate_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
