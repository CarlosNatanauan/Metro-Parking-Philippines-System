...
{
    public partial class UC_Weekly_detailed : UserControl
    {
        ...
        public UC_Weekly_detailed()
        {
            InitializeComponent();
            dtpWeekly.Format = DateTimePickerFormat.Custom;
            this.dtpWeekly.Value = DateTime.Now;
            dtpWeekly.CustomFormat = "MMMM dd, yyyy";
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            void PrintTitleAndHeaders()
            {
                ...
                {
                    subTitle = "WEEKLY REPORT FOR DISCOUNTED CUSTOMER";
                }
                else if (_currentReportType == ReportType.OverStay)
                {
                    subTitle = "WEEKLY REPORT FOR OVERSTYED CUSTOMER";
                }
                else
                {
                    subTitle = "WEEKLY DETAILED REPORT";
                }

                DateTime startDate = dtpWeekly.Value.Date;
                DateTime endDate = startDate.AddDays(6);

                string datePageNo = $"DATE : {startDate.ToString("MMM d, yyyy")} to {endDate.ToString("MMM d, yyyy")}                   PAGE NO: {_pageIndex}";
        ...
        {
            DataTable dataWithDiscount = databaseCodes.GetParkingLogDataWithDiscountWeekly(selectedDate);
        ...
        {
            DataTable dataWithOverstay = databaseCodes.GetParkingLogDataOverstayWeekly(selectedDate);

            // Check if the DataTable has any rows, which means there are vehicles with discounts
            btViewReportOverstay.Enabled = dataWithOverstay.Rows.Count > 0;
        }



        private void dtpWeekly_ValueChanged(object sender, EventArgs e)
        {
            setStatus();
            UpdateDiscountButtonStatus(dtpWeekly.Value);
            UpdateOverstayButtonStatus(dtpWeekly.Value);
        }

        private void btViewReportDiscount_Click(object sender, EventArgs e)
        {
            _currentReportType = ReportType.WithDiscount;
            DataTable data = databaseCodes.GetParkingLogDataWithDiscountWeekly(dtpWeekly.Value);
        ...
        private void btViewReport_Click(object sender, EventArgs e)
        {
            ...
            DataTable data = databaseCodes.GetParkingLogDataWeekly(dtpWeekly.Value);
            ShowPrintPreviewReport(data);
        }
        ...
        private void setStatus()
        {
            ...
            int totalVehicles = databaseCodes.GetParkingLogDataWeekly(dtpWeekly.Value).Rows.Count;

            // Get total discounted vehicles count
            int totalDiscountedVehicles = databaseCodes.GetParkingLogDataWithDiscountWeekly(dtpWeekly.Value).Rows.Count;
            int totalOverstayVehicles = databaseCodes.GetParkingLogDataOverstayWeekly(dtpWeekly.Value).Rows.Count;
            ...
            tbTotalVehicleOverstay.Text = totalOverstayVehicles.ToString();

            // Set status label and button availability based on the counts
            if (totalVehicles > 0)
            {
                lbStatusWeekly.Text = "AVAILABLE";
                lbStatusWeekly.ForeColor = Color.Green;
                ...
                btViewReportOverstay.Enabled = totalOverstayVehicles > 0;
            }
            else
            {
                lbStatusWeekly.Text = "NO VEHICLES";
                lbStatusWeekly.ForeColor = Color.Red;
                btViewReport.Enabled = false;
        ...
        private void btViewReportOverstay_Click(object sender, EventArgs e)
        {
            ...
            DataTable data = databaseCodes.GetParkingLogDataOverstayWeekly(dtpWeekly.Value);
            ShowPrintPreviewReport(data);
        }
    }
}
